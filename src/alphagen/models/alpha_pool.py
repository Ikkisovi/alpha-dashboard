...
class AlphaPool(AlphaPoolBase):
    ...
    def test_ensemble(self, data: StockData, target: Expression) -> Tuple[float, float]:
        with torch.no_grad():
            ...
            # Align all factors to minimum length before summing
            if factors:
                min_len = min(f.shape[0] for f in factors)
                factors = [f[-min_len:] for f in factors]
            
            combined_factor: Tensor = sum(factors)  # type: ignore
            target_factor = target.evaluate(data)
            
            # Align combined_factor and target_factor
            if combined_factor.shape[0] != target_factor.shape[0]:
                min_len = min(combined_factor.shape[0], target_factor.shape[0])
                combined_factor = combined_factor[-min_len:]
                target_factor = target_factor[-min_len:]
    ...
    def evaluate_ensemble(self):
        with torch.no_grad():
            if self.size == 0:
                return 0.0
            # Align values to minimum length before summing
            min_len = min(self.values[i].shape[0] for i in range(self.size) if self.values[i] is not None)
            aligned_sum = sum(self.values[i][-min_len:] * self.weights[i] for i in range(self.size) if self.values[i] is not None)
            ensemble_factor = self._normalize_by_day(aligned_sum)
            # Also align with target
            target_aligned = self.target[-min_len:] if self.target.shape[0] > min_len else self.target
            if ensemble_factor.shape[0] != target_aligned.shape[0]:
                min_len2 = min(ensemble_factor.shape[0], target_aligned.shape[0])
                ensemble_factor = ensemble_factor[-min_len2:]
                target_aligned = target_aligned[-min_len2:]
            ensemble_ic = batch_pearsonr(ensemble_factor, target_aligned).mean().item()
            return ensemble_ic
