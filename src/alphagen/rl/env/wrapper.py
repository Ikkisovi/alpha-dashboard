...
class AlphaEnvWrapper(gym.Wrapper):
    ...
    def step(self, action: int):
        # Verify action is masked as valid before executing
        mask = self.action_masks()
        if not mask[action]:
            # Invalid action selected by policy - truncate episode with negative reward
            # This is a workaround for MaskablePPO sometimes ignoring masks
            obs, _ = self.reset()
            return obs, -1.0, True, True, {"invalid_action": action}
    ...
    def action_masks(self) -> np.ndarray:
        ...
        # Validate each operator individually, not by category
        for i in range(OFFSET_OP, OFFSET_OP + SIZE_OP):
            op = OPERATORS[i - OFFSET_OP]
            if self.env._builder.validate_op(op):
                res[i - 1] = True
