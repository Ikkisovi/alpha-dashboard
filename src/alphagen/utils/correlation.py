...
def _rank_data(x: Tensor, nan_mask: Tensor) -> Tensor:
    """
    Memory-safe rank transform for Spearman correlation.

    Previous implementation built an O(d*s*s) equality tensor to average ranks for ties.
    That explodes memory for large cross-sections (e.g., 1500+ stocks).

    This version computes *average ranks for ties* in O(d*s) memory:
    - Sort each day's cross-section
    - Identify tie groups via adjacent comparisons
    - Compute each element's group's [start,end] indices using cumulative ops
    - Assign average rank = (start + end) / 2
    """
    # Sort cross-section per day: [d, s]
    sorted_vals, sorted_idx = torch.sort(x, dim=1)

    d, s = sorted_vals.shape
    idx = torch.arange(s, device=sorted_vals.device, dtype=sorted_idx.dtype).expand(d, s)

    # Group starts: True at first element and whenever value changes.
    new_group = torch.ones((d, s), dtype=torch.bool, device=sorted_vals.device)
    if s > 1:
        new_group[:, 1:] = sorted_vals[:, 1:] != sorted_vals[:, :-1]

    # For each position, carry forward the last start index in its tie group.
    start_candidates = torch.where(new_group, idx, torch.full_like(idx, -1))
    group_start = torch.cummax(start_candidates, dim=1)[0]

    # Group ends: True at last element and whenever next value changes.
    end_group = torch.ones((d, s), dtype=torch.bool, device=sorted_vals.device)
    if s > 1:
        end_group[:, :-1] = sorted_vals[:, :-1] != sorted_vals[:, 1:]

    # For each position, carry backward the next end index in its tie group.
    end_candidates = torch.where(end_group, idx, torch.full_like(idx, s))
    group_end_rev = torch.cummin(torch.flip(end_candidates, dims=[1]), dim=1)[0]
    group_end = torch.flip(group_end_rev, dims=[1])

    # Average rank for ties (0-based ranks).
    rank_sorted = (group_start + group_end).to(torch.float32) / 2.0

    # Scatter ranks back to original order.
    rank = torch.zeros((d, s), dtype=torch.float32, device=sorted_vals.device)
    rank.scatter_(1, sorted_idx, rank_sorted)

    # Mask NaNs (same behavior as before).
    rank[nan_mask] = 0
    return rank                                     # [d, s]
